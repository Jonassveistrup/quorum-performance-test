...
#def block_transactions(_block_number):
#	if _block_number == -1:
#		return 0
#	tx = w3.eth.getBlockTransactionCount(_block_number)
#	return tx
...
def block_transactions(_block_number):
	...
	if _block_number in blockNumbers:
		idx = blockNumbers.index(_block_number)
		print("idx: " + str(idx))
		return blockTransactionCounts[idx]
	else:
		blockNumbers.append(_block_number)
		tx = w3.eth.getBlockTransactionCount(_block_number)
		blockTransactionCounts.append(tx)
		print("numOfTx:" + str(tx))
		return tx
...
for tx in tx_to_block:
	...
	if t == 0:
		...
		res_tuple = (tx[0], tx[1], t*1000, tx[2], block_transactions(block_number) )
		tx_to_time.append(res_tuple)

f = open(sys.argv[3], 'w+')
for tx in tx_to_time:
	if tx[2] == 0:
		f.write(str(tx[0]) + ", " + str(tx[1]) +", " + str(tx[2]) + ", None, None\n")
	else:
